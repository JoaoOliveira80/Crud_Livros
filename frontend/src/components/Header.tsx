...
export default function Header({ onNovo }: HeaderProps) {
  ...
  const linkClass = (path: string) =>
    `text-sm font-medium transition-colors px-3 py-1 rounded-full ${
      pathname === path
        ? "text-primary bg-primary-container hover:bg-primary-container/90"
        ...
        <div className="max-w-5xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Button
              onClick={toggleTheme}
              variant="ghost"
              className="p-2 rounded-lg transition-colors hover:bg-surface-container-low text-on-surface-60 hover:text-primary focus-ring"
              aria-label={
                theme === "light"
